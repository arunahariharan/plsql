CREATE OR REPLACE PACKAGE "pk_no_rule_excptn_quote_idnfr"
AS
   PROCEDURE pr_no_rule_excptn_quote_idnfr (p_procedure_iid          IN     NUMBER,
...
END pk_no_rule_excptn_quote_idnfr;
/

CREATE OR REPLACE PACKAGE BODY "pk_no_rule_excptn_quote_idnfr"
AS
BEGIN
   PROCEDURE pr_no_rule_excptn_quote_idnfr (p_procedure_iid          IN     NUMBER,
                                 p_prcdr_ctgry_lkpcd   IN     VARCHAR2,
		...
		FROM  (SELECT src."PrvdrLctnXPrvdrLctnStSid",
			  ...
			  FROM
				( SELECT DISTINCT(PLXPLS.PRVDR_LCTN_X_PRVDR_LCTN_ST_SID) AS "PrvdrLctnXPrvdrLctnStSid",
				  ...
				  PD.NATIONAL_PRVDR_IDNTFR                               AS "Billing Provider NPI",
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
	  NULL;
  END pr_no_rule_excptn_quote_idnfr;
END pk_no_rule_excptn_quote_idnfr;
/